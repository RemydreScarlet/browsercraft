...
async function downloadFileToCheerpJ(url, destPath, progressCallback) {
  ...
  let bytes = new Uint8Array(downloaded);
  let pos = 0;
...
template.innerHTML = `
  <style>
    ...
    .display { width: 854px; height: 480px; position: absolute; inset: 0; visibility: hidden; }
    .intro { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; }
    p { max-width: 60ch; }
    .disclaimer { font-size: 0.8em; opacity: 0.5; }
    button { padding: 0.5em 1em; margin: 2em; }
    progress { width: calc(100% - 2em); margin: 1em; }
    *:focus { outline: none; }
  </style>
  ...
  <div class="intro">
    ...
    <p>Clicking the button below will download the client from mojang.com.
...
export default class MinecraftClient extends HTMLElement {
  ...
  async run() {
    ...
    const exitCode = await cheerpjRunMain(
      ...
      `/app/lwjgl-2.9.0.jar:/app/lwjgl_util-2.9.0.jar:${jarPath}`
    );
